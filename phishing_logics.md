# Phishing Logic & Workflow between content.js and popup.js
...
User opens an email in Gmail
        ↓
content.js detects the visible email
        ↓
content.js extracts sender, body, and links
        ↓
content.js runs phishing checks
        ↓
content.js calculates a score
        ↓
content.js converts score into a warning level
        ↓
content.js sends/stores the result for popup.js
        ↓
popup.js reads the result
        ↓
popup.js displays risk level, score, and warning message
```
...
- detect when a Gmail email is opened
- extract:
  - sender name
  - sender email
  - email body text
  - visible links
- apply simple phishing logic
- calculate a risk score
- generate warning messages
- save the final result to `chrome.storage.local` for `popup.js`

`popup.js` should:

- read the latest result from storage or message passing
- display:
  - warning level
  - risk score
  - detected suspicious signs
  - recommendation
- Other necessary functions to have beautiful UI/UX
...
`content.js` should write and return this format, `popup.js` should expect this input format
